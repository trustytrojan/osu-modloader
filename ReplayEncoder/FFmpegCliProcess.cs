...
public sealed class FFmpegCliProcess : IDisposable
{
	...
	public void Start()
	{
		...
		else if (OperatingSystem.IsLinux())
		{
			...
			var vaapiDevice = DetectVaapiDevice();
		...
		var videoArgs = $"-f rawvideo -pix_fmt rgba -s {(int)VideoSize.X}x{(int)VideoSize.Y} -r {Framerate} -i -";

		var audioPipeName = $"osu-framework-ffmpeg-audio-{Guid.NewGuid():N}";
		audioPipe = new NamedPipeServerStream(audioPipeName, PipeDirection.Out);

		var audioArgs = $"-f {SampleFormat} -ar {Samplerate} -ac {Channels} -i {toNamedPipePath(audioPipeName)}";
		var mappingArgs = "-map 0:v -map 1:a";
		...
		Task.Run(() =>
		{
			...
			{
				if (!videoQueue.Reader.TryRead(out var _image) || _image == null)
				{
					Thread.Yield();
					continue;
				}
		...
		Task.Run(() =>
		{
			...
			while (running || audioQueue.Reader.Count > 0)
			{
				...
				if (!audioPipe.IsConnected || !audioQueue.Reader.TryRead(out var audio) || audio == null)
				{
					Thread.Yield();
					continue;
				}
	...
	public bool WriteAudio(ReadOnlySpan<byte> audioData)
	{
		...
		if (ffmpegProcess.HasExited)
			throw new InvalidOperationException("ffmpeg has exited");
		var audioCopy = new byte[audioData.Length];
	...
	public static string DetectVaapiDevice()
	{
		...
		foreach (var filePath in Directory.GetFiles(dri_path))
		{
			...
			var arguments = $"-v warning -vaapi_device {filePath} " +

...
public static class GLRenderer_ExtractFrameBufferData_Patch
{
	...
	static readonly int numPbos = 2; // Double buffering
	static bool isPboInitialized = false;
	static int expectedByteSize = 0;
	...
	{
		var width = frameBuffer.Texture.Width;
		var height = frameBuffer.Texture.Height;
		var byteSize = width * height * 4;

		// 1. Initialize PBOs if not already done or if resolution changed
		if (!isPboInitialized || expectedByteSize != byteSize)
		{
		...
		var writeIdx = pboIndex % numPbos;
		var readIdx = (pboIndex + 1) % numPbos;
		...
		GL.BindBuffer(BufferTarget.PixelPackBuffer, pbos[writeIdx]);
		...
		GL.BindBuffer(BufferTarget.PixelPackBuffer, pbos[readIdx]);
	...
	{
		if (pbos != null) GL.DeleteBuffers(pbos.Length, pbos);

		pbos = new int[numPbos];
		GL.GenBuffers(numPbos, pbos);

...
{
	public static readonly Harmony Harmony = new(nameof(ReplayEncoder));
	public static readonly ReplayEncoder Instance = new();
	...
	void PostNotification(string text) => Game.PostNotification(new SimpleNotification() { Text = $"Replay Encoder: {text}" });

	void PostErrorNotification(string text) => Game.PostNotification(new SimpleErrorNotification() { Text = $"Replay Encoder: {text}" });
	...
	public void StartRecording(ScreenStack target)
	{
		...
		Harmony.PatchCategory("WhileRecording");

		var gameHost = AccessTools.Property(typeof(osu.Framework.Game), "Host").GetValue(Game) as GameHost;
		...
		if (Bass.ChannelGetInfo(myMixerHandle, out var info))
		...
		{
			var trackMixerHandle = Game.Audio.TrackMixer.GetBassHandle();
			var sampleMixerHandle = Game.Audio.SampleMixer.GetBassHandle();
		...
		else
		{
			...
			Harmony.PatchCategory("FakeGlobalMixerHandle");
			AccessTools.Method(typeof(AudioManager), "initCurrentDevice").Invoke(Game.Audio, []);
			Harmony.UnpatchCategory("FakeGlobalMixerHandle");
		}
	...
	public void StopRecording()
	{
		...
		if (Game.Audio.UseExperimentalWasapi.Value)
		{
			var trackMixerHandle = Game.Audio.TrackMixer.GetBassHandle();
			var sampleMixerHandle = Game.Audio.SampleMixer.GetBassHandle();
			var globalMixerHandle = Game.Audio.GetGlobalMixerHandle().Value
		...
		Harmony.UnpatchCategory("WhileRecording");
	...
	private void RecordAudio()
	{
		...
		if (audioBuf == null)
		{
			var afpvf = samplerate / fps;
			var samples = afpvf * channels;
			audioBuf = new byte[samples * resolution.ByteSize()];
		}

		audioTime.Begin();
		var bytesRead = Bass.ChannelGetData(myMixerHandle, audioBuf, audioBuf.Length);
		if (bytesRead == -1)
